...
grep -q '"readiness": "ready"' "$WORKDIR/status.json"
...
grep -q '"command": "mingsui connect"' "$WORKDIR/status.json"
"$bin" status -config "$cfg" -json=false >"$WORKDIR/status.txt"
grep -q '建议:' "$WORKDIR/status.txt"
grep -q '启动连接: mingsui connect' "$WORKDIR/status.txt"
